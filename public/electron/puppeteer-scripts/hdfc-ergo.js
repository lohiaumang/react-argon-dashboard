module.exports = async function (page, { data, modelData, dealerData }, mainWindow) {
  ...
  try {
    if (hdfcUsername && hdfcPassword) {
      await page.waitForSelector("#UserName", { visible: true });
      await waitForRandom();
      await page.type("#UserName", hdfcUsername);
      await page.waitForSelector("#Password", { visible: true });
      await page.type("#Password", hdfcPassword);
      await page.waitForSelector("button[type='submit']", { visible: true });
      await page.click("button[type='submit']");
      await page.waitForResponse("https://pie.hdfcergo.com/en-US/Login/AuthenticateUser");
      await waitForRandom();
      const headerText = await page.$eval("h2.ng-binding", el => el.textContent);
      if (headerText === "Warning") {
        await page.waitForSelector(".button-footer.text-center .btn.btn-primary[ng-click]", { visible: true });
        await page.click(".button-footer.text-center .btn.btn-primary[ng-click]");
        await page.waitForSelector("#Password", { visible: true });
        await page.type("#Password", hdfcPassword);
        await page.waitForSelector("button[type='submit']", { visible: true });
        await page.click("button[type='submit']");
    ...
    await page.waitForSelector(".product-section-slider a.tw-link[ng-click='ISQuickQuoteOrQuickProposal=1;showDashboardSearch=0;TWViewTitle()']", { visible: true });
    await waitForRandom();
    await page.click(".product-section-slider a.tw-link[ng-click='ISQuickQuoteOrQuickProposal=1;showDashboardSearch=0;TWViewTitle()']");
    mainWindow.webContents.send("update-progress-bar", ["20%", "insurance"]);
    await page.waitForSelector(".btn.btn-primary[ng-click='helpers.GoToUrl(TWURL)']", { visible: true });
    ...
    await page.type("input[name='ModelName']", data["Model Name"].split(" ")[0]);
    await waitForRandom();
    await page.waitForSelector(`li > a[title='${modelData[data["Model Variant"]].hdfcModelName}']`, { visible: true });
    await page.click(`li > a[title='${modelData[data["Model Variant"]].hdfcModelName}']`);
    await fetch_retry(`https://api.postalpincode.in/pincode/${data["Zip Code"]}`, {}, 5).then(async ([{ Status, PostOffice }]) => { // Fetching district and police station data
      if (Status === "Success" && PostOffice[0]) {
        ...
        await page.type("input[name='RtoLocation']", _("0.District", PostOffice).split(" ")[0]);
        await waitForRandom();
        await page.waitForSelector("input[name='RtoLocation']+ul > li > a", { visible: true });
