module.exports = function erp(page, { data, modelData, dealerData }, mainWindow) {
  ...
  async function automate() {
    ...
    const captcha = await page.$eval("#captchaCode", el => el.textContent.replace(/\s+/g, ''));
    await page.type("#s_captcha", captcha);
    await page.click("#s_swepi_22");
    await page.waitForSelector("#s_sctrl_tabScreen > ul > li > a", { visible: true });
    const tabButtons = await page.$$eval("#s_sctrl_tabScreen > ul > li > a", options => options.map(option => ({
      name: option.innerText,
      id: option.id
    })));
    const customerButton = tabButtons.find(button => button.name === "Customer");
